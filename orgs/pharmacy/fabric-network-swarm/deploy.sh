...
if [ -f "$ROOT_DIR/deploy.env" ]; then
    ...
    echo "Warning: deploy.env not found. Please copy deploy.env.example to deploy.env."
fi

# IPs and SSH users (Fallbacks for single-node mode)
...
# Fabric tooling (derive from .generated if not explicitly set)
FABRIC_BIN="${FABRIC_BIN:-$ROOT_DIR/.generated/fabric-samples/bin}"
FABRIC_CFG="${FABRIC_CFG:-$ROOT_DIR/.generated/fabric-samples/config}"
...
for PC_INFO in "$PC2_USER@$PC2_IP" "$PC3_USER@$PC3_IP"; do
    if [[ "$PC_INFO" == "@" || -z "${PC_INFO#*@}" ]]; then
        continue # skip if user or IP is unset for single-node mode
    fi
